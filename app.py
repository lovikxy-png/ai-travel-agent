...
def setup_page() -> None:
    ...
    *::after {
    ...
    /* Card truncation fix：标题、数字和带图标卡片必须完整换行显示。 */
    .card-title,
    .card-title-row h3,
    .advice-card h3,
    .tip-card h3,
...
def main() -> None:
    ...
    if submitted or force_regenerate:
        # generation_key：同一输入和同一模式对应的结果缓存 key。
        generation_key = build_generation_cache_key(user_input, generation_mode)
        if not user_input.strip():
            st.warning("请先输入一句旅行需求。")
        elif (
            not force_regenerate
            and st.session_state.get("last_result_data")
            and st.session_state["last_result_data"].get("generation_key") == generation_key
        ):
            st.session_state["last_result_data"]["reused_from_session"] = True
