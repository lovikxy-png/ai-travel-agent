...
def build_safe_key_slug(raw_text: str) -> str:
    """build_safe_key_slug：把按钮文案转换成适合 Streamlit key 使用的短标识。"""

    # slug_text：只保留中英文、数字和下划线，避免不同组件 key 冲突。
    slug_text = re.sub(r"[^\w\u4e00-\u9fff]+", "_", raw_text.strip(), flags=re.UNICODE).strip("_")
    return slug_text[:28] or "item"
...
请只返回合法 JSON，不要输出 Markdown，不要解释，不要使用代码块。
    JSON 顶层必须包含：
    trip_type, destination, destinations, total_days, days, nights, budget, preferences, trip_segments, daily_itinerary, food_recommendations, packing_checklist, photo_spots
...
必须包含 trip_type/destination/destinations/total_days/trip_segments/daily_itinerary/packing_checklist/photo_spots。
...
packing_checklist 必须包含 documents/payment_and_network/weather_and_clothing/electronics/medicine_and_emergency/photo_preparation/pre_departure_checks。
...
def build_markdown_from_structured_json(travel_json: dict, parsed_request: dict, facts_context: str) -> str:
    ...
    for food_item in travel_json.get("food_recommendations", []):
        food_title = food_item.get("name_cn", "美食点")
        if food_item.get("name_original") and food_item["name_original"] != food_title:
            food_title = f"{food_title}（{food_item['name_original']}）"
        recommended_dishes = food_item.get("recommended_dishes") or food_item.get("dishes") or "推荐菜请以菜单为准"
        food_lines.append(
            f"- {food_title}：地址 {food_item.get('location', '地址请以地图 App 最新结果为准')}｜"
            f"推荐 {recommended_dishes}｜{food_item.get('budget', '人均预算待确认')}｜"
    ...
    # packing_markdown/photo_markdown：从结构化 JSON 或确定性规则生成的新模块 Markdown。
...
def get_timeline_slot_config() -> list[dict]:
    """get_timeline_slot_config：返回每日时间线四个固定时间段配置。"""

    return [
        {"key": "morning", "label": "上午", "icon": "AM", "time": "09:00 - 11:30", "aliases": ["morning", "上午", "早上"]},
        {"key": "noon", "label": "中午", "icon": "NO", "time": "12:00 - 13:30", "aliases": ["noon", "中午", "午餐", "lunch"]},
        {"key": "afternoon", "label": "下午", "icon": "PM", "time": "14:00 - 17:30", "aliases": ["afternoon", "下午"]},
        {"key": "evening", "label": "晚上", "icon": "EV", "time": "18:30 - 21:30", "aliases": ["evening", "晚上", "夜晚", "night"]},
    ]
...
def normalize_timeline_slot(slot_data: object, slot_config: dict) -> dict:
    ...
    if isinstance(slot_data, dict):
        ...
        time_text = get_first_text_value(slot_data, ["time", "时间", "period", "time_range", "时段"]) or default_time
    ...
    return {
        ...
        "is_incomplete": not all([place_text, reason_text, duration_text, transport_text, booking_note]),
    }
...
def build_food_cards_from_json(travel_json: dict | None, parsed_request: dict) -> list[dict]:
    ...
    # food_cards：最终美食卡片数据。
    food_cards = []
    for food_item in food_recommendations:
        if not isinstance(food_item, dict):
            continue
...
def render_input_box() -> tuple[bool, str]:
    ...
    with st.container(border=True):
        ...
        for index, sample_prompt in enumerate(SAMPLE_PROMPTS):
            # sample_key：示例按钮唯一 key，避免移动端重排时组件 key 冲突。
            sample_key = f"sample_prompt_{index}_{build_safe_key_slug(sample_prompt['label'])}"
            if sample_columns[index].button(sample_prompt["label"], key=sample_key):
...
def render_food_cards(
    ...
    for destination in parsed_request.get("destinations", [parsed_request["destination"]]):
        ...
        for food in destination_cards[:6]:
            original_name_html = (
                f'<div class="food-map-keyword">{html.escape(food["name_original"])}</div>'
